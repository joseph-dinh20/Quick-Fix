...
const expandedFrameworks: Framework[] = []

for (let i = 1; i <= 10; i++) {
  for (const framework of frameworks) {
    expandedFrameworks.push({
      value: `${framework.value}-${i}`,
      label: `${framework.label} ${i}`,
    })
  }
}
...
const selectedFramework = computed(() =>
  ...
  expandedFrameworks.find(framework => framework.value === value.value),
)
      ...
      <Command>
        ...
        <CommandList>
          ...
          <CommandGroup>
            ...
            <CommandItem v-for="framework in expandedFrameworks" :key="framework.value" :value="framework.value"
              @select="(ev) => {
