        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-1 flex flex-col gap-1">
              <p class="text-sm text-slate-500">{{ job.customer?.name || job.customer || 'Unknown customer' }}</p>
              <p class="text-sm text-slate-500 capitalize">{{ job.request_type || 'Request type not provided' }}</p>
            </div>
            <div class="mt-3">
              <span class="text-sm font-bold text-slate-900">
                {{ job.budget ? `$${job.budget}` : '$' + job.price || '$0' }}
              </span>
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
              ...
              <div class="flex items-center text-slate-600 font-medium capitalize">
                ...
                {{ selectedJob.city + ', ' + `${selectedJob.state || 'California'}` || 'Request type not provided' }}
              </div>
              ...
              <div class="flex items-center text-slate-600 font-medium">
                ...
                {{ formatDate(selectedJob.deadline || selectedJob.date) || 'No deadline provided' }}
              </div>
            ...
            <div class="mt-6 pt-2">
              <span class="text-lg font-bold text-[#1a202c]">
                {{ selectedJob.budget ? `$${selectedJob.budget}` : '$' + selectedJob.price|| '$0' }}
              </span>
...
export default {
  ...
  data() {
    return {
      ...
      favoritedJobs: computed(() => useJobStore().getFavoritedJobs()),
      jobs: [],
      loading: false,
      selectedJob: null,
      isDialogOpen: false,
  ...
  methods: {
    ...
    async fetchJobs() {
      ...
      try {
        ...
        console.error("Backend failed, using fallback data", err);
        this.jobs = this.favoritedJobs.map((job, index) => this.fallbackJob(job, index));
      } finally {
        this.loading = false;
      }
    },
