...
// Live-merged ratings (seed + user-added) for this provider.
const mergedRatings = computed(() =>
  ratingsStore.getRatingsFor(provider.userID),
);
const totalRating = computed(() => mergedRatings.value.length);
const liveAverageRating = computed(
  () => ratingsStore.getAverageFor(provider.userID) || "—",
);
...
const chunkUserRating = computed(() => {
  const ratingList = mergedRatings.value;
  const chunks = [];
      ...
      <CardFooter class="flex flex-col">
        <div class="items-start self-start w-full h-auto">
          ...
          <Card
            ...
            :key="rating.userName + rating.date"
            class="my-4 p-4"
          >
